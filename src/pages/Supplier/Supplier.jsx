...
import { SupplierContext } from "../../contexts/SupplierContext"; // Importa o contexto
...
export default function Supplier() {
  ...
  // Acessa a função que limpa o erro
  const { fetchSupplierByNameOrCode, getSuppliers, fetchSuppliers, setError } = useContext(SupplierContext);
  ...
  const handleSearchClick = async () => {
    try {
      ...
      const result = await fetchSupplierByNameOrCode(searchTerm);
  ...
  const tableData = searchResults !== null ? searchResults : getSuppliers || [];
    ...
    <div className="p-2">
      ...
      <div className="bg-white dark:bg-gray-700 text-zinc-700 dark:text-zinc-300 rounded-xl mt-5 p-4">
        <motion.div
          ...
          transition={{ duration: 0.5 }}
        >
          <Link to="/supplier/create" onClick={() => { setError(null) }} className="flex items-center gap-3 my-5 curso-pointer"
          >
            <button
